...
function Toast({ message, visible }) {
  return (
    <div style={{
      ...
      boxShadow: "0 8px 24px rgba(26,107,90,0.3)", zIndex: 200, pointerEvents: "none",
    }}>✓ {message}</div>
  );
}
...
function CheckinPage({ patients, setPatients, showToast }) {
  ...
  const todayCheckins = patients.flatMap((p) =>
    p.checkins.filter((c) => new Date(c.ts).toDateString() === today()).map((c) => ({ ...c, patient: p }))
  ).sort((a, b) => b.ts - a.ts);
  ...
  const handleCheckin = () => {
    ...
    if (existing) {
      setPatients((prev) => prev.map((p) =>
        p.id === existing.id ? { ...p, checkins: [newCheckin, ...p.checkins] } : p
      ));
    } else {
      setPatients((prev) => [...prev, {
        id: genId(), firstName: firstName.trim(), lastName: lastName.trim(),
        phone: phone.trim(), checkins: [newCheckin],
      }]);
    }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      {/* Form */}
      <div style={{ background: colors.card, borderRadius: 20, boxShadow: "0 4px 32px rgba(26,107,90,0.07)", padding: 28 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16, marginBottom: 16 }}>
          <Input label="First Name" value={firstName} onChange={setFirstName} placeholder="Ahmad" autoFocus />
          <Input label="Last Name" value={lastName} onChange={setLastName} placeholder="Khalil" />
          <Input label="Phone" value={phone} onChange={setPhone} placeholder="+962 7X XXX XXXX" />
        </div>
        {error && <div style={{ color: colors.error, fontSize: 13, fontFamily: font, marginBottom: 12 }}>{error}</div>}
        <button onClick={handleCheckin} style={{
          width: "100%", padding: "13px", borderRadius: 12, background: colors.primary,
          color: "white", border: "none", fontSize: 16, fontFamily: font, fontWeight: 700,
      ...
      {/* Today list */}
      <div style={{ background: colors.card, borderRadius: 20, boxShadow: "0 4px 32px rgba(26,107,90,0.07)", overflow: "hidden" }}>
...
function RecordsPage({ patients, setPatients, showToast }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      {/* Search */}
      <div style={{ background: colors.card, borderRadius: 16, padding: "16px 20px", boxShadow: "0 2px 16px rgba(26,107,90,0.06)" }}>
          ...
          filtered.map((p, i) => (
            <div key={p.id} style={{
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <span style={{
                  fontFamily: font, fontSize: 12, color: colors.textLight,
                  background: colors.bg, padding: "3px 10px", borderRadius: 8,
                }}>{p.checkins.length} visit{p.checkins.length !== 1 ? "s" : ""}</span>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {[...historyPatient.checkins].sort((a, b) => b.ts - a.ts).map((c, i) => (
                <div key={c.id} style={{
                  ...
                  <span style={{ fontFamily: font, fontSize: 15, color: colors.text, fontWeight: 500 }}>
                    {fmtDate(c.ts)}
                  </span>
                  <span style={{ fontFamily: font, fontSize: 13, color: colors.textLight }}>
                    {fmtTime(c.ts)}
                  </span>
                </div>
              ))}
  ...
  return (
    <div style={{
      ...
      <div style={{ maxWidth: 760, margin: "0 auto", display: "flex", flexDirection: "column", gap: 24 }}>
        ...
        {/* Nav */}
        <div style={{ background: colors.card, borderRadius: 14, padding: 6, display: "flex", gap: 4, boxShadow: "0 2px 12px rgba(26,107,90,0.06)" }}>
